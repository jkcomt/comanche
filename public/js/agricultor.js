...
$('input[name=ruc]').on('keyup',function(event){
    ...
    if($longitud > 0)
    {
        $('#registrarAgricultor input[name=dni]').prop('readonly',true);
    }else{
        $('#registrarAgricultor input[name=dni]').prop('readonly',false);
    }
});
...
$('input[name=dni]').on('keyup',function(event){
    ...
    if($longitud > 0)
    {
        $('#registrarAgricultor input[name=ruc]').prop('readonly',true);
    }else{
        $('#registrarAgricultor input[name=ruc]').prop('readonly',false);
    }
});

$('#registrarAgricultor').submit(function(event)
{
    event.preventDefault();
...
$('#update').on('click',function (e) {
    ...
    var url = $('#agri-frm-update').attr('action');
...
$('#buscarAgricultor').on('keyup',function(){
   valor = $(this).val()

    // e.preventDefault();
    var token = $('input[name=_token]').attr('value')

    var url = "/agricultor/buscar"
    $.ajax({
        type:"get",
        headers: {'X-CSRF-TOKEN':token},
        url:url,
